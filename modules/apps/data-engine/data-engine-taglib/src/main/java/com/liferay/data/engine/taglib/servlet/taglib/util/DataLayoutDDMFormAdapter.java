...
public class DataLayoutDDMFormAdapter {
	...
	private DDMForm _getDDMForm() throws Exception {
		...
		jsonObject = jsonObject.put(
			...
			"defaultLanguageId", _dataLayout.getDefaultLanguageId()
		);

		_transformOptions(jsonObject);
	...
	private void _transformOptions(JSONObject jsonObject) {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			if (fieldJSONObject.has("options")) {
				...
				fieldJSONObject.put(
					"options", _createOptionsJSONArray(options));
			}
		}
	}
