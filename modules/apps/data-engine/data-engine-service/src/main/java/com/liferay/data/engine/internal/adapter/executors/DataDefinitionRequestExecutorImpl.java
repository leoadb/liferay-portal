...
import com.liferay.data.engine.service.constants.DEServiceErrorCode;
...
public class DataDefinitionRequestExecutorImpl
	...
	public DeleteDataDefinitionResponse execute(
		...
		try {
			...
			throw new DataEngineException(
				Arrays.asList(
					new DataEngineAdapterError(
						DEServiceErrorCode.NO_SUCH_DATA_DEFINITION,
						errorMessage)),
				nsse);
		}
		catch (Exception e) {
			...
			throw new DataEngineException(
				Arrays.asList(
					new DataEngineAdapterError(
						DEServiceErrorCode.UNABLE_TO_DELETE_DATA_DEFINITION,
						errorMessage)),
				e);
		}
	}
	...
	protected SaveDataDefinitionResponse insert(
		...
		try {
			...
			_resourceLocalService.addModelResources(
				deDataDefinition.getCompanyId(), groupId,
				PrincipalThreadLocal.getUserId(),
				_portal.getClassName(deDataDefinition.getClassNameId()),
	...
	protected SaveDataDefinitionResponse update(
		...
		try {
			...
			_modelResourcePermission.check(
				PermissionThreadLocal.getPermissionChecker(),
				deDataDefinition.getDEDataDefinitionId(), ActionKeys.UPDATE);
			...
			throw new DataEngineException(
				Arrays.asList(
					new DataEngineAdapterError(
						DEServiceErrorCode.UNABLE_TO_SAVE_DATA_DEFINITION,
						errorMessage)),
				e);
		}
	}
